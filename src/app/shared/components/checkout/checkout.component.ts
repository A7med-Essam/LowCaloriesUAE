...
import {
  ICheckOutPrice,
  ICustomCheckOut,
  ICustomCheckOut_WithAuth,
  IGiftCode,
  INormalCheckOut,
  INormalCheckOut_WithAuth,
...
export class CheckoutComponent implements OnInit, AfterViewInit {
  ...
  @Input() SubscribtionData!:
    | any
    | ISubscriptionData_NormalPlan
  ...
  ngAfterViewInit(): void {
    ...
    this._AuthService.currentUser.subscribe((res) => {
      if (!res) {
        this.LocationBtn?.nativeElement?.setAttribute(
          'data-bs-target',
          '#LoginModal'
        );
        this.isLogin = false;
  ...
  applyGiftCode(GiftCodeInput: HTMLInputElement) {
    ...
    this._CheckoutService
      .applyGiftCode(giftCodeFormData)
  ...
  deleteLocation() {
    this._LocationService
      .deleteAddress(this.SelectedLocationId)
      .subscribe((res) => {
        this.getAddresses();
      });
  }
    ...
    if (this.OldAddressForm.valid) {
      if (this.plan_name == 'custom') {
        ...
        this._CheckoutService
          ...
          .subscribe((res: IDefaultResponse) => {
        ...
        const SubData: INormalCheckOut_WithAuth = {
          ...
          subscription_from: 'web',
          meal_backend_types: this.SubscribtionData.meal_types.filter(
            (item: ISubscriptionData_NormalPlan) =>
              !this.SubscribtionData.SnacksType.includes(item)
          ),
          snack_backend_types: this.SubscribtionData.SnacksType,
        };
        this._CheckoutService
          .getNormalCheckOut_WithAuth(SubData)
          .subscribe((res: IDefaultResponse) => {
            window.location.href = res.data;
          });
      }
      ...
      if (this.translate.currentLang == 'ar') {
        this._ToastrService.error('فشلت محاولة الدفع', 'تنبيه', {
          timeOut: 4000,
        });
      } else {
        this._ToastrService.error('CheckOut attempt failed', 'Notification', {
          timeOut: 4000,
        });
      }
    }
  ...
  getCheckOut_WithOutAuth(NewAddressForm: FormGroup) {
    if (NewAddressForm.valid) {
      if (this.plan_name == 'custom') {
        ...
        this._CheckoutService
          .getNormalCheckOut_WithOutAuth(SubData)
          .subscribe((res: IDefaultResponse) => {
            window.location.href = res.data;
          });
      }
