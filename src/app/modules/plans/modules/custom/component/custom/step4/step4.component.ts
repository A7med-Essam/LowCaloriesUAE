...
import { CustomPlan_Menu_CustomOptions2,CustomPlan_step4_Dates_CustomOptions, Plan_Categories_CustomOptions } from 'src/app/shared/models/ngx-owlcarousel';
import { faCalendarAlt, faEdit, faArrowsRotate } from '@fortawesome/free-solid-svg-icons';
...
export class Step4Component implements OnInit {
  ...
  toggleDates(e:HTMLElement){
    ...
    const Siblings = this._SharedService.getAllSiblings(e.parentElement?.parentElement, e.parentElement?.parentElement?.parentElement);
  ...
  ngOnInit(): void {
    ...
    this.MealsPerDay = Number(this.SubscribtionDetailsData?.meal_count) + Number(this.SubscribtionDetailsData?.snack_count);
    this.getMeals();
    this.getCards();
    if (this.translate.currentLang == 'ar') {
  ...
  getCards(){
    this.translate.currentLang == 'ar'? this.Cards.map( c => c.day = c.day_ar):false;
  }

  ChangeCurrentIndex(e:number){
    this.CurrentIndex = (Number(this.SubscribtionDetailsData?.meal_count) + Number(this.SubscribtionDetailsData?.snack_count)) * e;
    this.MealsPerDay = this.CurrentIndex + (Number(this.SubscribtionDetailsData?.meal_count) + Number(this.SubscribtionDetailsData?.snack_count));
  }
  ...
  OnCloseModal(){
    ...
    let m = this.ClientMeals.find(x=>x.UniqueId == this.MealDetails.UniqueId);
    if (m) {
  ...
  removeItem(arr:IMealDetailsUnique[], value:IMealDetailsUnique, newItem:IMealDetailsUnique) {
